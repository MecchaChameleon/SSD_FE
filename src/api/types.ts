...
export type Reservation = { id:number; product:Pick<Product,'id'|'name'> & {businessName?:string}; buyerId?:number; buyerNickname?:string; quantity:number; unitPrice:number; totalAmount:number; visitStartAt:string|null; visitEndAt?:string|null; status:ReservationStatus; rejectReason?:string|null; requestedAt:string };
