...
export type Product = { id:number; sellerProfileId?:number; name:string; description:string; businessName?:string; businessType:BusinessType; category:ProductCategory; type?:'INDOOR'|'OUTDOOR'|null; totalQty:number; qty:number; price:number; minPrice:number; currentPrice:number; discountRate?:number; openTime:string|null; deadline:string; address:string|null; lat:number|null; lng:number|null; distanceMeters?:number; urgent?:boolean; aiInsight?:string; imageUrls?:string[]; status:ProductStatus; createdAt:string; updatedAt:string };
export type ProductInput = { name:string; description:string; businessType:BusinessType; category:ProductCategory; type?:'INDOOR'|'OUTDOOR'; qty:number; price:number; minPrice:number; openTime?:string; deadline:string; foot?:'HIGH'|'LOW'; address?:string|null; lat?:number|null; lng?:number|null; status?:ProductStatus };
export type Purchase = { id:number; productId:number; productName:string; businessName?:string; buyerId?:number; buyerNickname?:string; quantity:number; originalPrice:number; unitPrice:number; totalAmount:number; status:PurchaseStatus; rejectReason?:string|null; requestedAt:string; paymentStatus:'PAID'|'REFUNDED' };
