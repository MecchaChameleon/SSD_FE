...
import AlertIcon from "../../icon/x.svg";
...
export function ReservationScreen({
  ...
  const submit = async () => {
    ...
    try {
      ...
      await buyerApi.reserve({
        productId: product.id,
        quantity: Number(quantity),
        visitStartAt: date.toISOString(),
      });
      setDone(true);
    } catch (error) {
      setRequestError(
        error instanceof ApiError ? error.message : "예약 요청에 실패했습니다.",
      );
    } finally {
      setSaving(false);
    }
  ...
  if (done)
    return (
      <View style={s.root}>
        <View style={s.completeArt}>
          <Character width={112} height={112} />
        </View>
        <View style={s.completeCopy}>
          <Text style={s.completeTitle}>예약 요청 완료!</Text>
          <Text style={s.completeBody}>
            판매자는 요청된 예약을 승인 또는 거절할 수 있어요.
          </Text>
        </View>
        <Pressable
          style={s.completeButton}
          onPress={() => onComplete(Number(quantity))}
        >
  ...
  return (
    <View style={s.root}>
      ...
      <View style={s.section}>
        ...
        <ReservationProduct product={product} />
      </View>
      <View style={s.form}>
        <Text style={s.formTitle}>예약 정보 작성</Text>
        <Labeled label="수량">
          <TextInput
        ...
        <Labeled label="구매 시각">
          <Pressable
            onPress={openPicker}
            style={[s.input, s.timeInput, !!time && s.activeInput]}
          >
            <Text style={[s.inputText, !time && s.placeholder]}>
              {time || "선택"}
            </Text>
      ...
      <View style={s.notice}>
        <AlertIcon width={14} height={14} color={colors.g500} />
        <Text style={s.noticeText}>
          예약한 구매 시각이 지났음에도 방문하지 않으면 노쇼처리 됩니다.
        </Text>
      </View>
      ...
      <Pressable
        ...
        onPress={submit}
        style={[s.submit, (!valid || saving) && s.submitDisabled]}
      >
        <Text style={[s.buttonText, (!valid || saving) && s.disabledText]}>
          {saving ? "요청 중..." : "예약 요청하기"}
        </Text>
      </Pressable>
      <Modal
...
function ReservationProduct({ product }: { product: Product }) {
  return (
    <View style={[s.product, { height: 190 }]}>
      ...
      <View style={s.meta}>
        <Text style={s.shop}>{product.shop}</Text>
        <Text style={s.location}>{product.location}</Text>
      </View>
      <View style={s.meta}>
        <Text style={s.location}>{product.detail}</Text>
        {product.urgent ? (
          <View style={s.tag}>
            <Text style={s.tagText}>마감임박</Text>
          </View>
        ) : null}
      </View>
      <View style={s.price}>
        <Text style={s.original}>{product.original}</Text>
        <Text style={s.sale}>{product.price}</Text>
      </View>
...
const s = StyleSheet.create({
  ...
  discount: { fontSize: 16, fontWeight: "600", color: colors.info },
  meta: { flexDirection: "row", alignItems: "center", gap: 8 },
  shop: { fontSize: 14, fontWeight: "600", color: colors.g800 },
  location: { fontSize: 12, color: colors.g600 },
  tag: {
    paddingHorizontal: 6,
    paddingVertical: 4,
    borderRadius: 4,
  ...
  sale: { fontSize: 20, fontWeight: "600", color: colors.danger },
  remaining: { fontSize: 12, color: colors.g600 },
  form: { paddingTop: 24, paddingHorizontal: 16, gap: 16 },
  formTitle: {
    fontSize: 18,
    fontWeight: "600",
    marginBottom: 0,
  ...
  required: { fontSize: 16, color: colors.primary500 },
  input: {
    height: 52,
    borderWidth: 1,
    borderColor: colors.g300,
  ...
  activeInput: { borderColor: colors.primary500 },
  ...
  noticeText: { fontSize: 10, color: colors.g500 },
  requestError: {
    position: "absolute",
    left: 16,
    right: 16,
  ...
  submitDisabled: { backgroundColor: colors.g200 },
  ...
  completeArt: { position: "absolute", top: "25%", alignSelf: "center" },
  completeCopy: {
    position: "absolute",
    top: "44%",
    left: 16,
  ...
  completeTitle: { fontSize: 18, fontWeight: "600", color: colors.black },
  completeBody: { fontSize: 16, color: colors.g600, textAlign: "center" },
  completeButton: {
    position: "absolute",
    top: "63.5%",
    left: 16,
