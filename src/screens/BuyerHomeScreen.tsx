...
export function BuyerHomeScreen({
  ...
  const [visitTime,setVisitTime]=useState('오후 9:00');
  ...
  if(detailProduct) return <BuyerProductDetail product={detailProduct} liked={liked.includes(detailProduct.id)} onBack={()=>setDetailProduct(null)} onLike={()=>toggleLike(detailProduct.id)} onBuy={()=>{setQuantity(1);setPurchase(detailProduct);setCheckout('order');setDetailProduct(null)}}/>;
...
function OrderForm({product,quantity,visitTime,onQuantity,onVisitTime,onBack,onNext}:{product:Product;quantity:number;visitTime:string;onQuantity:(v:number)=>void;onVisitTime:(v:string)=>void;onBack:()=>void;onNext:()=>void}){
  const [picker,setPicker]=useState(false);const times=['오후 5:00','오후 6:00','오후 7:00','오후 8:00','오후 9:00','오후 10:00'];
  return <View style={checkoutStyles.root}><CheckoutHeader title="주문하기" onBack={onBack}/><ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={checkoutStyles.content}><Text style={checkoutStyles.sectionTitle}>상품 정보</Text><CheckoutProduct product={product}/><Text style={[checkoutStyles.sectionTitle,{marginTop:24}]}>주문 정보 작성</Text><Text style={checkoutStyles.label}>수량<Text style={checkoutStyles.required}> *</Text></Text><TextInput value={String(quantity)} onChangeText={v=>onQuantity(Math.max(1,Math.min(Number(product.remaining.replace(/[^0-9]/g,''))||99,Number(v.replace(/\D/g,''))||1)))} keyboardType="number-pad" style={checkoutStyles.input}/><Text style={checkoutStyles.label}>방문 시각<Text style={checkoutStyles.required}> *</Text></Text><Pressable onPress={()=>setPicker(true)} style={checkoutStyles.input}><Text style={checkoutStyles.inputText}>{visitTime}</Text><ChevronDownIcon width={24} height={24} color={colors.g400}/></Pressable></ScrollView><View style={checkoutStyles.bottom}><Pressable onPress={onNext} style={checkoutStyles.primaryButton}><Text style={checkoutStyles.primaryText}>다음</Text></Pressable></View><Modal transparent visible={picker} animationType="slide"><Pressable style={checkoutStyles.sheetOverlay} onPress={()=>setPicker(false)}><View style={checkoutStyles.timeSheet}><View style={checkoutStyles.handle}/><Text style={checkoutStyles.sheetTitle}>방문 시각 선택</Text>{times.map(time=><Pressable key={time} onPress={()=>{onVisitTime(time);setPicker(false)}} style={checkoutStyles.timeItem}><Text style={[checkoutStyles.timeText,time===visitTime&&checkoutStyles.timeSelected]}>{time}</Text></Pressable>)}</View></Pressable></Modal></View>
}
...
function BuyerProductDetail({product,liked,onBack,onLike,onBuy}:{product:Product;liked:boolean;onBack:()=>void;onLike:()=>void;onBuy:()=>void}) {
  ...
  return <View style={detailStyles.root}>
    ...
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={detailStyles.panel}>
