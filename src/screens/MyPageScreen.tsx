...
type Business = { shop: string; number1: string; number2: string; number3: string; address: string; bank: string; account: string; latitude?:number|null; longitude?:number|null };
...
const profileToBusiness = (profile: SellerProfile): Business => { const numbers=profile.businessNumber.split('-'); return {shop:profile.businessName,number1:numbers[0]??'',number2:numbers[1]??'',number3:numbers[2]??'',address:profile.address??'',bank:profile.bankName??'',account:profile.accountNumber??'',latitude:profile.latitude,longitude:profile.longitude}; };
...
export function MyPageScreen({ initialBusinessRegistration=false, onBusinessRegistered, onHome, onMap, onPurchases, onSellerMode, onLogout, onWithdraw }: { initialBusinessRegistration?:boolean; onBusinessRegistered?:()=>void; onHome: () => void; onMap: () => void; onPurchases: () => void; onSellerMode: () => void; onLogout: () => Promise<void>; onWithdraw: () => Promise<void> }) {
  ...
  if (page === 'businessForm') return <BusinessForm initial={business ?? initialBusiness} editing={!!business} onBack={() => initialBusinessRegistration?onHome():setPage(formReturn)} onSave={async value => { if(!business)await sellerApi.apply({businessName:value.shop,businessNumber:`${value.number1}-${value.number2}-${value.number3}`,representativeName:name,businessDocumentUrl:''});await sellerApi.createProfile({address:value.address,latitude:value.latitude??null,longitude:value.longitude??null,bankName:value.bank,accountNumber:value.account,accountHolder:name});setBusiness(value);if(initialBusinessRegistration&&onBusinessRegistered){onBusinessRegistered();return}setPage(business ? 'business' : 'businessDone'); }} />;
...
export function BusinessRegistrationScreen({name,onBack,onComplete}:{name:string;onBack:()=>void;onComplete:()=>void}) {
  return <BusinessForm initial={initialBusiness} editing={false} sellerSignup onBack={onBack} onSave={async value=>{
    await sellerApi.apply({businessName:value.shop,businessNumber:`${value.number1}-${value.number2}-${value.number3}`,representativeName:name,businessDocumentUrl:''});
    await sellerApi.createProfile({address:value.address,latitude:value.latitude??null,longitude:value.longitude??null,bankName:value.bank,accountNumber:value.account,accountHolder:name});
    onComplete();
  }}/>;
}
...
function BusinessForm({ initial, editing, sellerSignup=false, onBack, onSave }: { initial: Business; editing: boolean; sellerSignup?:boolean; onBack: () => void; onSave: (value: Business) => void }) {
  ...
  const valid = useMemo(() => [value.shop,value.number1,value.number2,value.number3,value.address,value.bank,value.account].every(Boolean) && value.number1.length === 3 && value.number2.length === 2 && value.number3.length === 5, [value]);
