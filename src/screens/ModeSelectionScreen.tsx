...
export function ModeSelectionScreen({onComplete}:{onComplete:(mode:ServiceMode)=>void}) {
  ...
  return <View style={[s.root, { paddingTop: topInset + 48 }]}>
    ...
    <View style={s.titleBlock}>
      ...
      <Text style={s.helper}>ⓘ 모드는 최초 1회 사업자 인증 후 앱 내 설정에서 언제든지 변경 가능합니다.</Text>
    </View>
    <View style={s.cards}>
    ...
    <View style={s.button}>
      <ActionButton disabled={!mode} onPress={()=>mode&&onComplete(mode)}>다음</ActionButton>
    </View>
  </View>;
}
...
function ModeCard({title,description,selected,onPress}:{title:string;description:string;selected:boolean;onPress:()=>void}) {
  return (
    <Pressable accessibilityRole="radio" accessibilityState={{checked:selected}} onPress={onPress} style={[s.card,selected&&s.cardSelected]}>
...
const s=StyleSheet.create({
  ...
  logo:{width:133,height:61,marginLeft:-10,marginBottom:16},
  titleBlock:{gap:6,marginBottom:36},
  title:{fontSize:18,fontWeight:'600',lineHeight:21,color:colors.black},
  helper:{fontSize:11,lineHeight:14,color:colors.g400},
  cards:{gap:14,marginBottom:44},
  card:{height:120,borderWidth:1,borderColor:colors.g200,borderRadius:16,paddingHorizontal:16,justifyContent:'center'},
  cardSelected:{borderColor:colors.primary500},
  cardTitle:{fontSize:20,lineHeight:24,fontWeight:'600',color:colors.black},
  cardDescription:{marginTop:8,fontSize:13,lineHeight:18,color:colors.g500},
  button:{},
});
